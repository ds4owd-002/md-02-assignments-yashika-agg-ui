...
```{r}
ggplot(data = gapminder_summary_continent_year, mapping = aes(x = year, y = lifeExp, color = continent)) +
  geom_line() +
  geom_point()
...
ggplot(data = gapminder_summary_2007,
       mapping = aes(x = continent, y = count)) +geom_col()
```
...
*Your answer here: It will change the bin width and might affect the data visualization*

```{r}
ggplot(data = gapminder_2007, 
       mapping = aes(x = lifeExp, fill = continent)) +
  geom_histogram(col = "grey30", breaks = seq(30, 65, 6))
```
...
```{r}
ggplot(data = gapminder_2007, mapping = aes(
  x = gdpPercap,
  y = lifeExp,
  size = pop,
  color = country
